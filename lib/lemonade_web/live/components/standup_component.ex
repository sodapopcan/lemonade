    ...
    <section class="mb-4 group">
      ...
      <div class="flex items-center h-20">
        <%= for standup_member <- @standup.standup_members do %>
          <div class="<%= if standup_member.team_member_id in @present_team_member_ids, do: "border border-green-400" %> <%= if standup_member.on_vacation, do: "opacity-50" %> w-20 h-20 bg-yellow-300 rounded-full centered text-2xl mr-2 shadow-md"><%= initials(standup_member.name) %></div>
        <% end %>
        <%= if !attending_standup?(@current_team_member, @standup) do %>
