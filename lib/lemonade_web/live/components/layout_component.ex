  ...
  def render(assigns) do
    ...
    <div class="w-screen" x-data="{mainTitle: 'Lemonade'}">
      <div class="flex items-center bg-yellow-500 py-2 px-3 text-sm">
        <div class="flex-1">
          <%= if @team do %>
            <h1 class="text-base"><%= @organization.name %> - <%= @team.name %></h1>
          <% else %>
            <h1 class="text-base" x-html="mainTitle">Lemonade</h1>
          <% end %>
        </div>
        <p><%= @current_organization_member.name %></p>

        <div class="relative" x-data="{ open: false }">
          <a href="#" @click="open = true" class="bg-yellow-400 w-6 h-6 rounded-full p-0 ml-2 text-xs centered"><%= initials(@current_organization_member.name) %></a>
